...
**Run one drain at a time per `--run_id`.** Two concurrent drains on the same
run id do not split the queue between them — they both read the same rows and
...
(Postgres only, and not wired to the CLI) — see
...
emits `FOR UPDATE SKIP LOCKED`, and the test suite asserts the clause is
generated, but generating it is not the same as watching two consumers claim
disjoint batches on a live Postgres. Until someone does that, a single drain per
