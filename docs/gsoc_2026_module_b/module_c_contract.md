...
- **Concurrency — single consumer per run today.** The orchestrator serialises A→B→C and runs **one** Module C consumer per `pipeline_run_id`, and `DbKnowledgeSource` reads **without** row locking (`session.query(...).filter(consumed_at IS NULL, llm_label IN (…)).order_by(created_at, id)`) — safe for a single consumer. Running **multiple concurrent** consumers over the same rows is **not** safe as-is: two would select the same rows before either stamps `consumed_at`. To support that, each consumer must claim rows atomically — `SELECT ... WHERE consumed_at IS NULL ... FOR UPDATE SKIP LOCKED`, held through mapping + persistence + the `consumed_at` update in one transaction (this schema has only `consumed_at`, no claim-token column). `DbKnowledgeSource` now takes that claim **opt-in**, via `lock_rows=True` (plumbed through `run_librarian_queue`), which adds `FOR UPDATE SKIP LOCKED`; it is **off by default**, because a single consumer gains nothing from paying for the lock. Three properties matter to B:
  - **Postgres only, and it refuses rather than degrades.** SQLAlchemy's SQLite compiler emits an *empty* `for_update_clause`, so SQLite does not error on `FOR UPDATE` — it silently drops it, handing back an unlocked batch to a caller who asked for a locked one. C therefore raises on any non-Postgres dialect instead. (SQLite dev/CI is single-consumer regardless; the `NOW()` in the snippets is illustrative — C sets `consumed_at` via the ORM, backend-portable.)
  - **The lock is held for the length of the batch,** because it has to be: the claim only holds if one transaction covers mapping, persistence, and the `consumed_at` update. That means rows in `knowledge_queue` stay locked while C runs embedding and cross-encoder inference over them, so pair `lock_rows` with a `limit` small enough that a batch is minutes, not hours. The lock-free alternative is the claim-token column this schema lacks (`claimed_at` / `claimed_by`), which would let a row be claimed in a short transaction and processed outside any lock — a B-side schema change, and worth weighing before C is scaled horizontally in earnest.
  - **A locked batch can come back short.** Postgres applies `LIMIT` to the scan and *then* drops rows another consumer holds, so a consumer may receive fewer rows than `limit` — sometimes none — while unclaimed rows remain. A short batch means "keep draining", not "the queue is empty".
- **If the single-consumer rule were violated without the lock, the cost is wasted compute, not corruption.** Module C's `decision_queue` is unique on `(chunk_id, pipeline_run_id)` and the `consumed_at` update is filtered on `IS NULL`, so a doubled batch converges on the same rows and the same decisions; what it burns is the embedding and cross-encoder passes. Consumers on *different* `pipeline_run_id`s never race at all, since the read is scoped by run id and `run_librarian_queue` refuses a blank one. Filed and traced in #1025.
