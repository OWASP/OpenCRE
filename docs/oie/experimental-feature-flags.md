...
This branch preserves librarian levers and experiment YAMLs that were evaluated
on the OIE B2 harness and **not** promoted for the exact Links bar. The promoted
stack on `wip/oie-pipeline-eval-harness` / mainline docs is only:
...
Implementation modules remain in `application/utils/librarian/` on both branches
(flag-gated, default off) so research can be re-run without resurrecting deletes.
This branch is the **documentation and experiment-config home** for non-winners.
