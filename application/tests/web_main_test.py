...
class TestMain(unittest.TestCase):
    ...
    def test_all_cres_pages_deterministically(self) -> None:
        ...
        for external_id in reversed(external_ids):
            collection.add_cre(defs.CRE(name=f"cre-{external_id}", id=external_id))
        collection.session.commit()
    ...
    def test_all_cres_orders_duplicate_external_ids_by_primary_key(self) -> None:
        """external_id is not unique on its own: the constraint is on
        (name, external_id), so two CREs may carry the same one. Ordering by
        external_id alone would leave those ties undefined, which is what the
        CRE.id half of the sort is for. The primary keys are fixed here rather
        than generated, so the expected order cannot drift between runs."""
        collection = db.Node_collection().with_graph()
        ...
        with self.app.test_client() as client:
            response = client.get(
                "/rest/v1/all_cres?per_page=10&page=1",
                headers={"Content-Type": "application/json"},
            )
            self.assertEqual(200, response.status_code)
