...
class TestConfigLoaderDefaults(unittest.TestCase):
    def test_defaults_when_env_unset(self):
        ...
        self.assertFalse(cfg.use_rrf)
        self.assertFalse(cfg.context_enrich)
...
class TestConfigLoaderOverrides(unittest.TestCase):
    OVERRIDES: ClassVar[Dict[str, str]] = {
        ...
        "CRE_LIBRARIAN_USE_RRF": "1",
        "CRE_LIBRARIAN_CONTEXT_ENRICH": "true",
    ...
    def test_env_overrides_apply(self):
        ...
        self.assertTrue(cfg.use_rrf)
        self.assertTrue(cfg.context_enrich)
    ...
    def test_margin_gamma_defaults_to_085_when_boolean_set(self):
        with mock.patch.dict(
            os.environ, {"CRE_LIBRARIAN_MARGIN_GAMMA": "on"}, clear=True
        ):
            cfg = load_config()
        self.assertAlmostEqual(cfg.margin_gamma or 0.0, 0.85)

    def test_margin_gamma_does_not_affect_hybrid_gamma(self):
        with mock.patch.dict(
            os.environ,
            {
                "CRE_LIBRARIAN_MARGIN_GAMMA": "0.9",
                "CRE_LIBRARIAN_HYBRID_GAMMA": "0.70",
            },
            clear=True,
        ):
            cfg = load_config()
        self.assertAlmostEqual(cfg.margin_gamma or 0.0, 0.9)
        self.assertAlmostEqual(cfg.hybrid_gamma, 0.70)

    def test_bad_margin_gamma_raises(self):
        with mock.patch.dict(
            os.environ, {"CRE_LIBRARIAN_MARGIN_GAMMA": "1.5"}, clear=True
        ):
