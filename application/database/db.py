...
class CRE_Graph:
    ...
    def instance(cls, session):
        if cls.__instance is None:
            ...
            cls.cre_graph = cls.__load_cre_graph(session)
    ...
    def add_edge(self, *args, **kwargs):
        return self.cre_graph.add_edge(*args, **kwargs)

    def add_node(self, *args, **kwargs):
        return self.cre_graph.add_node(*args, **kwargs)

    @classmethod
    def add_cre_graph_node(cls, dbcre: CRE, graph: nx.DiGraph) -> nx.DiGraph:
        if dbcre:
            graph.add_node(
    ...
    @classmethod
    def add_node_graph_node(cls, dbnode: Node, graph: nx.DiGraph) -> nx.DiGraph:
        if dbnode:
            graph.add_node(
                "Node: " + str(dbnode.id),
    ...
    def __load_cre_graph(cls, session) -> nx.Graph:
        ...
        for il in session.query(InternalLinks).all():
            ...
            graph = cls.add_cre_graph_node(dbcre=group, graph=graph)

            cre = session.query(CRE).filter(CRE.id == il.cre).first()
            if not cre:
                logger.error(f"CRE {il.cre} does not exist?")
            graph = cls.add_cre_graph_node(dbcre=cre, graph=graph)
        ...
        for lnk in session.query(Links).all():
            ...
            graph = cls.add_node_graph_node(dbnode=node, graph=graph)
...
class Node_collection:
    ...
    def __init__(self) -> None:
        self.cre_graph = CRE_Graph.instance(session=sqla.session)
    ...
    def __introduces_cycle(self, node_from: str, node_to: str) -> Any:
        try:
            existing_cycle = nx.find_cycle(self.cre_graph.cre_graph)
            if existing_cycle:
                logger.fatal(
        ...
        new_graph = self.cre_graph.cre_graph.copy()
    ...
    def add_cre(self, cre: cre_defs.CRE) -> CRE:
        ...
        if entry is not None:
            ...
            self.session.commit()
            self.cre_graph = self.cre_graph.add_cre_graph_node(
    ...
    def add_node(self, node: cre_defs.Node) -> Optional[Node]:
        ...
        if entries:
            ...
            self.cre_graph = self.cre_graph.add_node_graph_node(
    ...
    def find_path_between_nodes(
        ...
        res: bool = nx.has_path(
            self.cre_graph.cre_graph.to_undirected(),
    ...
    def get_root_cres(self):
        ...
        nodes = [
            node
            for node in self.cre_graph.cre_graph.nodes
            if self.cre_graph.cre_graph.in_degree(node) == 0 and node.startswith("CRE")
        ]
        result = []
        for nodeid in nodes:
            result.extend(
                self.get_CREs(
                    internal_id=self.cre_graph.cre_graph.nodes[nodeid]["internal_id"]
                )
            )
        return result
