...
class PromptHandler:
    ...
    def get_id_of_most_similar_node(self, standard_text_embedding: List[float]) -> str:
        ...
        if not self.existing_node_embeddings:
            (
                ...
                self.database.get_embeddings_by_doc_type(cre_defs.Credoctypes.Standard.value)
            )
    ...
    def get_id_of_most_similar_cre_paginated(self, item_embedding: List[float]) -> str:
        ...
        embeddings, total_pages = self.database.get_embeddings_by_doc_type_paginated(
            cre_defs.Credoctypes.CRE.value, page=0
        )
        max_similarity = -1
    ...
    def get_id_of_most_similar_node_paginated(
        ...
        embeddings, total_pages = self.database.get_embeddings_by_doc_type_paginated(
            cre_defs.Credoctypes.Standard.value, page=0
        )
        max_similarity = -1
        most_similar_index = 0
        most_similar_id = ""
        ...
        for page in range(total_pages):
            ...
            if np.max(similarities) > max_similarity:                
                max_similarity = np.max(similarities)
                print(f"found higher max similarity of {max_similarity}")

                most_similar_index = np.argmax(similarities)
                most_similar_id = existing_standard_ids[most_similar_index]
            embeddings, _ = self.database.get_embeddings_by_doc_type_paginated(
