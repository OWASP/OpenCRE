...
def zap_alert(name:str,id:str,description:str,tags:List[str],code:str)->defs.Tool:
    return defs.Tool(tooltype=defs.ToolTypes.Offensive, name=f"ZAP Alert: {name}",id=id,description=description,tags=tags,hyperlink=code)
                ...
                for node in cwe_nodes:
                    for link in node.links:
                        if link.document.doctype == defs.Credoctypes.CRE:
                            alert = zap_alert(name=name,id=externalId,description=description,tags=[tag],code=code)
                            dbnode = cache.add_node(alert)
                            cache.add_link(cre=db.dbCREfromCRE(link.document),node=dbnode)
