...
| `CRE_LIBRARIAN_DUAL_INDEX` | off | Header→name stubs, body→summaries; requires summaries for the body pool |
| `CRE_LIBRARIAN_USE_RRF` | off | Fuse dual-index pools with RRF (k=60) instead of max-score + name quota |
| `CRE_LIBRARIAN_CONTEXT_ENRICH` | off | Prepend `Standard:` / `Section:` when missing before retrieve/CE |
...
| `CRE_LIBRARIAN_MARGIN_GAMMA` | off | Keep rank-1; keep i>1 iff `score_i ≥ γ·score_1` (default γ=0.85 when set). Not `HYBRID_GAMMA` |
| `CRE_LIBRARIAN_HUB_LEAF_CAGE` | off | High-risk: top hubs then Contains children allowlist |
