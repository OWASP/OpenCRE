    ...
    *,
    ...
    if summary_vectors:
        ...
        if config.dual_index and cre_embeddings:
            ...
            logger.info(
                "CRE_LIBRARIAN_DUAL_INDEX: header→%s names, body→%s summaries%s",
                len(cre_embeddings),
                len(summary_vectors),
                " (RRF)" if config.use_rrf else "",
            )
        else:
    ...
    if config.hub_leaf_cage and parent_index is not None:
        ...
        retriever = HubLeafCageRetriever(retriever, parent_index)
        logger.info("CRE_LIBRARIAN_HUB_LEAF_CAGE: hub→Contains-children cage on")
