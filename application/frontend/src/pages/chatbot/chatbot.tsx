...
export const Chatbot = () => {
  ...
  function onSubmit() {
    setLoading(true);
    setChatMessages(chatMessages => [...chatMessages, { timestamp: new Date().toLocaleTimeString(), role: "user", message: chat.term, data: [] }])
    ...
    fetch(`${apiUrl}/completion`, {
      ...
      setLoading(false);
      setError('')
      setChatMessages(chatMessages => [...chatMessages, { timestamp: data.timestamp, role: "assistant", message: data.response, data: data.table }])
    })
      .catch((error) => {
  ...
  function displayDocument(d: Document) {
    return (
      <a href={d.hyperlink} target="_blank">*Reference: The above answer was based on the {d.name} section of {d.section};</a>
    )
  }
  return (
    <>
                  ...
                  <div id="chat-messages">
                    {chatMessages.map((m) => (
                      <div id={m.message} className='foobar'>
                        ...
                        <Comment.Group className={m.role == 'user' ? "right floated six wide column" : "left floated six wide column"}>
                          <Comment>
                            <Comment.Content>
                              ...
                              {m.data ? m.data?.map((m2) => { return displayDocument(m2) }) : ""}
                            </Comment.Content>
                          </Comment>
                        </Comment.Group>
                      </div>
