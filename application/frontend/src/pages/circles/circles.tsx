...
import { event, hierarchy, interpolateHcl, interpolateZoom, pack, scaleLinear, select, transition } from 'd3';
...
const RenderCircle = () => {
  ...
  function populateChildren(id) {
    ...
    if (cre)
      cre.links
        ?.filter((link) => link.ltype === TYPE_CONTAINS)
        .forEach((link) => {
          let child: any = getById(link.document.id);
          if (child) {
            cre.links?.push({ document: child, ltype: TYPE_CONTAINS });
          }
  ...
  var text = g
    ...
    .text(function (d) {
      let name =
        d.data.name.length > 33
          ? d.data.name.substr(0, 14) + '...' + d.data.name.substr(d.data.name.length - 14)
          : d.data.name;
