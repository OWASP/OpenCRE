...
class TestCreDefs(unittest.TestCase):
    ...
    def test_document_to_dict(self):
        ...
        standard_output = {'doctype': 'Standard', 'hyperlink': None, 'name': 'ASVS',
                           'section': 'SESSION-MGT-TOKEN-DIRECTIVES-DISCRETE-HANDLING', 'subsection': '3.1.1'}

        cre = defs.CRE(
            id="100", description="CREdesc", name="CREname", links=[defs.Link(document=standard)], tags=['CREt1', 'CREt2'])
        ...
        group = defs.CRE(id="500", description="desc", name="name", links=[defs.Link(
            document=cre), defs.Link(document=standard2)], tags=['tag1', 't2'])
        ...
        nested = defs.Standard(doctype=defs.Credoctypes.Standard, name='ASVS',
                               section='SESSION-MGT-TOKEN-DIRECTIVES-DISCRETE-HANDLING', subsection='3.1.1')
        nested_output = {'doctype': 'Standard', 'hyperlink': None, 'name': 'ASVS',
                         'section': 'SESSION-MGT-TOKEN-DIRECTIVES-DISCRETE-HANDLING', 'subsection': '3.1.1'}
