...
class TestMain(unittest.TestCase):
    ...
    def test_register_standard_with_links(self):
        standard_with_links = defs.Standard(doctype=defs.Credoctypes.Standard, id='', description='', name='standard_with_links',
                                            links=[
                                                defs.Link(document=defs.Standard(doctype=defs.Credoctypes.Standard, id='', description='', name='CWE', links=[
                                                ], tags=[], metadata=defs.Metadata(labels=[]), section='598')),
                                                defs.Link(document=defs.Standard(doctype=defs.Credoctypes.Standard, id='', description='', name='ASVS', links=[
    ...
    def test_register_standard_with_cre(self):
        ...
        standard_with_cre = defs.Standard(doctype=defs.Credoctypes.Standard, id='', description='', name='standard_with_cre',
                                          links=[
                                              defs.Link(document=defs.CRE(doctype=defs.Credoctypes.CRE, id='', description='cre desc', name='crename', links=[
        ...
        self.assertEqual(len(self.collection.session.query(
            db.Links).all()), 2)  # 2 links in the db
        self.assertEqual(len(self.collection.session.query(
            db.Standard).all()), 2)  # 2 standards in the db
        self.assertEqual(len(self.collection.session.query(
    ...
    def test_register_standard_with_groupped_cre_links(self):
        with_groupped_cre_links = defs.Standard(doctype=defs.Credoctypes.Standard, id='', description='', name='standard_with',
                                                links=[
                                                    defs.Link(document=defs.CRE(id='', description='group desc', name='group name', links=[
                                                        defs.Link(document=defs.CRE(doctype=defs.Credoctypes.CRE, id='101-001', description='cre2 desc', name='crename2', links=[], tags=[], metadata=defs.Metadata(labels=[])))], tags=[], metadata=defs.Metadata(labels=[]))),
                                                    defs.Link(document=defs.Standard(doctype=defs.Credoctypes.Standard, id='', description='', name='CWE', links=[
                                                    ], tags=[], metadata=defs.Metadata(labels=[]), section='598')),
                                                    defs.Link(document=defs.CRE(doctype=defs.Credoctypes.CRE, id='', description='cre desc', name='crename', links=[
                                                    ], tags=[], metadata=defs.Metadata(labels=[]))),
    ...
    def test_register_cre(self):
        ...
        cre = defs.CRE(id="100", description="CREdesc", name="CREname", links=[
                       defs.Link(document=standard)], tags=['CREt1', 'CREt2'], metadata=defs.Metadata(labels=['CREl1', 'CREl2']))
    ...
    def test_parse_file(self):
        ...
        expected = defs.CRE(doctype=defs.Credoctypes.CRE, id='001-005-073', description='Verify that approved cryptographic algorithms are used in the generation, seeding, and verification.', name='CREDENTIALS_MANAGEMENT_CRYPTOGRAPHIC_DIRECTIVES',
                            links=[
                                defs.Link(document=defs.Standard(doctype=defs.Credoctypes.Standard, id='', description='', name='TOP10', links=[], tags=[], metadata={
                                }, section='https://owasp.org/www-project-top-ten/2017/A5_2017-Broken_Access_Control', subsection='None', hyperlink='None')),
                                defs.Link(document=defs.Standard(doctype=defs.Credoctypes.Standard, id='', description='', name='ISO 25010', links=[], tags=[], metadata={}, section='Secure data storage', subsection='None', hyperlink='None'))], tags=[], metadata={})
    ...
    def test_parse_standards_from_spreadsheeet(self):
        input = [{'ASVS Item': 'V1.1.1', 'ASVS-L1': '', 'ASVS-L2': 'X', 'ASVS-L3': 'X',
                  'CORE-CRE-ID': '002-036', 'CRE Group 1': 'SDLC_GUIDELINES_JUSTIFICATION',
                  'CRE Group 1 Lookup': '925-827', 'CRE Group 2': 'REQUIREMENTS',
                  'CRE Group 2 Lookup': '654-390', 'CRE Group 3': 'RISK_ANALYSIS',
                  'CRE Group 3 Lookup': '533-658', 'CRE Group 4': 'THREAT_MODEL',
                  'CRE Group 4 Lookup': '635-846', 'CRE Group 5': '',
                  'CRE Group 5 Lookup': '', 'CRE Group 6': '',
                  'CRE Group 6 Lookup': '', 'CRE Group 7': '',
                  'CRE Group 7 Lookup': '',
                  'CWE': 0,
                  ...
                  'PL-8 SECURITY AND PRIVACY ARCHITECTURES', 'NIST 800-63': 'None',
                  'OPC': 'C1', 'SIG ISO 25010': '@SDLC', 'Top10 2017': '',
                  'UNIQUE IDs\n\nRandom CRE-ID PER ASVS\n=concatenate(randbetween(100,999),"-",randbetween(100,999))': '418-852',
                  'WSTG': 'test', 'cheat_sheets': 'https: // cheatsheetseries.owasp.org/cheatsheets/Threat_Modeling_Cheat_Sheet.html, https: // cheatsheetseries.owasp.org/cheatsheets/Abuse_Case_Cheat_Sheet.html, https: // cheatsheetseries.owasp.org/cheatsheets/Attack_Surface_Analysis_Cheat_Sheet.html'}]
