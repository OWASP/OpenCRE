...
log() {
  echo "[push-local-postgres-to-heroku-staging] $*"
}

die() {
  echo "[push-local-postgres-to-heroku-staging] ERROR: $*" >&2
  exit 1
}

# Required:
#   STAGING_APP                  Heroku app name (e.g. opencre-staging)
#
# Optional:
...
#   SYNC_PROD_DATABASE_URL       1 to set PROD_DATABASE_URL to staging DB (default: 1)
...
if [[ "${CAPTURE_BACKUP}" == "1" ]]; then
  log "Capturing staging backup before restore"
  heroku pg:backups:capture -a "${STAGING_APP}"
fi

log "Resolving staging DATABASE_URL"
STAGING_DB_URL="$(heroku config:get DATABASE_URL -a "${STAGING_APP}")"
[[ -n "${STAGING_DB_URL}" ]] || die "Failed to resolve staging DATABASE_URL for ${STAGING_APP}"
...
detect_source_pg_major() {
  ...
  # server_version_num: major*10000 + minor*100 + patch (e.g. 180003)
  echo $((ver_num / 10000))
}
...
if [[ -z "${PG_CLIENT_IMAGE}" ]]; then
  if major="$(detect_source_pg_major "postgres:18")"; then
    ...
    # Safe modern fallback when probing fails.
...
log "Restoring dump into Heroku staging via Docker pg_restore"
if [[ "${RESET_TARGET_PUBLIC_SCHEMA}" == "1" && "${SCOPE_ALL}" == "1" ]]; then
  ...
  docker run --rm --network host \
    "${PG_CLIENT_IMAGE}" \
    psql "${STAGING_DB_URL_SSL}" -v ON_ERROR_STOP=1 -c "DROP SCHEMA IF EXISTS public CASCADE; CREATE SCHEMA public;"
...
restore_cmd+=(--dbname "${STAGING_DB_URL_SSL}" "${dump_dir}/${dump_base}")
"${restore_cmd[@]}"

if [[ "${SYNC_PROD_DATABASE_URL}" == "1" ]]; then
  log "Syncing PROD_DATABASE_URL to staging DATABASE_URL (SQLAlchemy-safe)"
  SQLA_URL="${STAGING_DB_URL/#postgres:\/\//postgresql://}"
  heroku config:set -a "${STAGING_APP}" "PROD_DATABASE_URL=${SQLA_URL}" >/dev/null
fi

if [[ "${SCOPE_ALL}" == "1" ]]; then
  log "Done. Staging DB replaced from local Postgres source."
else
  log "Done. Staging DB updated from local Postgres source (SYNC_TABLES=${SYNC_TABLES})."
fi
log "Quick verify: heroku pg:info -a ${STAGING_APP}"
